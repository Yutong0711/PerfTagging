...
public class KafkaRules {
	...
	String[] rules = { "gk1_efficiency", "gk2_complexity", "gk3_reduce", "gk4_performance", "gk5_latency",
			...
			"sk14_overhead", "sk15_network", "sk16_garbage", "sk17_workload", "sk18_throttle", "sk19_processing_time",
			"sk1_negative_necessary", "sk2_block_hang_crash", "sk3_byte", "sk4_prefetch", "sk5_hotspot",
			"sk6_enough_space", "sk7_buffer", "sk8_timeout_expiration", "sk9_frequency", "ss10_called_frequently",
			"ss11_rework", "ss12_spend_time", "ss13_cache", "ss14_perform", "ss15_each_one", "ss16_close_immediately",
			"ss17_cardinality_cost", "ss18_fast_to", "ss19_lot_reading", "ss1_one_per", "ss20_roundtrip",
			"ss21_traversal", "ss22_cd_files", "ss23_avoid_if", "ss24_cost", "ss25_sessions",
			"ss26_negative_do_everything", "ss27_algorithm", "ss28_waste", "ss29_save_time_space", "ss2_all_to_one",
			"ss30_decrease_operations", "ss3_in_one_go", "ss4_memory", "ss5_cpu", "ss6_per_per", "ss7_nn_by_nn",
			"ss8_load_nn", "ss9_respond_until", "tk10_binary_search", "tk11_power_consumption", "tk12_stress_testing",
			"tk13_huge_data_chunk", "tk14_data_rate", "tk15_GC_pressure", "tk16_big_O", "tk17_disk_spills",
			"tk18_brute_force", "tk1_contended_lock", "tk2_bound", "tk3_memory_unit", "tk4_cpi", "tk5_execution_skew",
			"tk6_iteration", "tk7_concurrent", "tk8_probe_filter", "tk9_thread_pause" };
	...
	public static List<Sentence> checkRules(List<Sentence> sentences) {
		...
		for (Sentence sentence : sentences) {
			...
			if (sk17_dynamic_allocation(sentence)) {
				numbers.add(26);
			...
			if (ss27_algorithm(sentence)) {
				numbers.add(77);
				names.add("ss27_algorithm");
			}
			if (ss28_waste(sentence)) {
				numbers.add(78);
				names.add("ss28_waste");
			}
			if (ss29_save_time_space(sentence)) {
				numbers.add(79);
				names.add("ss29_save_time_space");
			}
			if (ss30_decrease_operations(sentence)) {
				numbers.add(80);
				names.add("ss30_decrease_operations");
			}
	...
	public static boolean sk17_dynamic_allocation(Sentence sentence) {
		String text = sentence.getText().toLowerCase();
	...
	public static boolean ss11_rework(Sentence sentence) {
		...
		for (String word : words) {
			if (word.equals("redo") || word.contains("recalculat") || word.contains("rework") || word.startsWith("re-")
					...
					|| word.contains("reload") || word.equals("resize") || word.equals("resizing"))
				return true;
		}
		return false;
	}
	...
	public static boolean ss27_algorithm(Sentence sentence) {
		List<String> words = sentence.getWords();
		boolean b1 = false, b2 = false;
		for (String word : words) {
	...
	public static boolean ss28_waste(Sentence sentence) {
		List<String> words = sentence.getWords();
		boolean b1 = false, b2 = false;
		for (String word : words) {
	...
	public static boolean ss29_save_time_space(Sentence sentence) {
		List<String> words = sentence.getWords();
		boolean b1 = false, b2 = false;
		for (String word : words) {
	...
	public static boolean ss30_decrease_operations(Sentence sentence) {
		List<String> words = sentence.getWords();
		boolean b1 = false, b2 = false;
		for (String word : words) {
